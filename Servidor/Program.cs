                ...
                case 2:
                    {
                        if (dados.Count > 0)
                        {
                            ...
                            var totalVendas = filtrados.Sum(x => x.valorVendido);
                            data = $"O total de vendas da loja {msg.codigoLoja} foi de: {totalVendas}.";
                        }
                        break;
                    }
                case 3:
                    {
                        if (dados.Count > 0)
                        {
                            ...
                            var totalVendas = filtrados.Sum(x => x.valorVendido);
                            data = $"O total de vendas da rede entre {msg.dataInicial} e {msg.dataFinal} foi de: {totalVendas}.";
                        }
                        break;
                    }
                case 4:
                    {
                        if (dados.Count > 0)
                        {
                            var filtrados = dados.GroupBy(x => x.nomeVendedor).Select(x => x.FirstOrDefault()).ToList();
